...
pub fn compare_size(item_b: &Box<FileNode>, item_a: &Box<FileNode>) -> cmp::Ordering {
    // REVERSED FOR DESCENDING ORDER
    item_a.size.cmp(&item_b.size)
}

pub fn compare_name(item_a: &Box<FileNode>, item_b: &Box<FileNode>) -> cmp::Ordering {
...
pub fn sort_items(items: &mut Vec<Box<FileNode>>, options: &Options) -> () {
    match options.sort {
        None => return,
